...
def test_demo_with_scripted_input():
    ...
    try:
        ...
        assert (
            "✓ HPXML created" in result.stdout
            or "✓ Simulation complete" in result.stdout
            or "Converting H2K to HPXML..." in result.stdout
            or "Missing dependencies" in result.stdout
        ), "Demo should start conversion or report dependency issues"
        ...
        assert result.returncode in [
            0,
            1,
        ], f"Demo should complete successfully or with warnings, got: {result.returncode}"
...
"""
    ...
    try:
        ...
        if result.returncode != 0:
            pytest.fail(
                f"Conversion test failed with return code {result.returncode}\nSTDOUT: {result.stdout}\nSTDERR: {result.stderr}"
            )
