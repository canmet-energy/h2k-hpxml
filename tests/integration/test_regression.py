...
def test_regression(check_openstudio_bindings):
    ...
    ), f"Baseline summary file not found: {baseline_summary_file}"
    ...
    for source_file, baseline_info in baseline_summary["results"].items():
                ...
                if current_energy_data:
                    ...
                    if energy_comparisons:
                        ...
                        if energy_failed == 0:
                            ...
                            print(
                                f"✅ Energy validation PASSED: {energy_passed}/{len(energy_comparisons)} comparisons"
                            )
                        else:
                            ...
                            print(
                                f"❌ Energy validation FAILED: {energy_passed}/{len(energy_comparisons)} comparisons passed"
                            )
                        ...
                        energy_comparison_results["files_failed"] += 1
                        combined_results["energy_failures"] += 1
                        print("❌ Energy validation FAILED: No valid comparisons generated")
                else:
                    file_result["energy_status"] = "FAIL"
                    file_result["status"] = "FAIL"
                    energy_comparison_results["files_failed"] += 1
                    combined_results["energy_failures"] += 1
                    print("❌ Energy validation FAILED: No energy data extracted")
            else:
                file_result["energy_status"] = "FAIL"
                file_result["status"] = "FAIL"
                energy_comparison_results["files_failed"] += 1
                combined_results["energy_failures"] += 1
                print("❌ Energy validation FAILED: SQL file not found")
        else:
            file_result["energy_status"] = "FAIL"
            file_result["status"] = "FAIL"
            energy_comparison_results["files_failed"] += 1
            combined_results["energy_failures"] += 1
            print("❌ Energy validation FAILED: No baseline energy data")
        ...
        if current_hpxml_path:
            ...
            if os.path.exists(baseline_hpxml_path):
                ...
                hpxml_comparison_results["files_failed"] += 1
                combined_results["hpxml_failures"] += 1
                print("❌ HPXML validation FAILED: Baseline HPXML not found")
        else:
            file_result["hpxml_status"] = "FAIL"
            file_result["status"] = "FAIL"
            hpxml_comparison_results["files_failed"] += 1
            combined_results["hpxml_failures"] += 1
            print("❌ HPXML validation FAILED: Current HPXML not found")
    ...
    combined_summary_path = (
        "tests/fixtures/expected_outputs/golden_files/comparison/comparison_combined_summary.json"
    )
    try:
        import json
    ...
    if combined_results["files_failed"] > 0:
        pytest.fail(
            f"Regression failed for {combined_results['files_failed']} files: {failed_files}"
        )
