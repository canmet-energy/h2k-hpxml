    ...
    try:
        ...
        if building is not None:
            ...
            extracted_data["building_info"] = {
                "site_type": site.find(f'.//{namespace}SiteType').text if site is not None and site.find(f'.//{namespace}SiteType') is not None else None,
                "building_type": building_details.find(f'.//{namespace}BuildingType').text if building_details is not None and building_details.find(f'.//{namespace}BuildingType') is not None else None,
                "conditioned_floor_area": building_details.find(f'.//{namespace}ConditionedFloorArea').text if building_details is not None and building_details.find(f'.//{namespace}ConditionedFloorArea') is not None else None,
                "conditioned_building_volume": building_details.find(f'.//{namespace}ConditionedBuildingVolume').text if building_details is not None and building_details.find(f'.//{namespace}ConditionedBuildingVolume') is not None else None,
                "number_of_bedrooms": building_details.find(f'.//{namespace}NumberofBedrooms').text if building_details is not None and building_details.find(f'.//{namespace}NumberofBedrooms') is not None else None,
                "number_of_bathrooms": building_details.find(f'.//{namespace}NumberofBathrooms').text if building_details is not None and building_details.find(f'.//{namespace}NumberofBathrooms') is not None else None,
            }
        
        # Enclosure components (walls, windows, doors, etc.)
        enclosure_elements = [
            'Wall', 'Window', 'Door', 'Floor', 'Slab', 'Ceiling', 'Roof'
        ]
        ...
        # HVAC Systems
        hvac_systems = [
            'HeatingSystem', 'CoolingSystem', 'HeatPump', 'HVACDistribution'
        ]
        ...
        if climate_elem is not None:
            ...
            extracted_data["climate"] = {
                "weather_station_name": weather_station.find(f'.//{namespace}Name').text if weather_station is not None and weather_station.find(f'.//{namespace}Name') is not None else None,
                "weather_station_wmo": weather_station.find(f'.//{namespace}WMO').text if weather_station is not None and weather_station.find(f'.//{namespace}WMO') is not None else None,
            }
    ...
    try:
        ...
        if not files_match:
            ...
            if len(meaningful_diffs) > 20:
                differences = meaningful_diffs[:20] + [f"... and {len(meaningful_diffs) - 20} more differences"]
            else:
                differences = meaningful_diffs
                
            # If no meaningful differences found, indicate files differ but in subtle ways
            if not differences:
                differences = ["Files differ but differences are subtle (whitespace, formatting, etc.)"]
...
def validate_hpxml_structure(hpxml_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if building is not None:
            ...
            for element in required_building_elements:
                if building.find(f'.//{namespace}{element}') is None:
                    validation_result["warnings"].append(f"Recommended Building element missing: {element}")
        ...
        validation_result["structure_info"] = {
            ...
            "has_enclosure": any(root.findall(f'.//{namespace}{elem}') for elem in ['Wall', 'Window', 'Door']),
            "has_systems": any(root.findall(f'.//{namespace}{elem}') for elem in ['HeatingSystem', 'CoolingSystem', 'WaterHeatingSystem'])
        }
