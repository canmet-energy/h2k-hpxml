    ...
    try:
        ...
        for row in tabular_results:
            ...
            try:
                ...
                print(f"Skipping non-numeric value: '{clean_value}' for {report_name}/{table_name}/{row_name}/{column_name}")
...
def extract_from_hpxml_reports(cursor: sqlite3.Cursor) -> Dict[str, Any]:
    ...
    try:
        ...
        energy_data["HPXML_Reports"] = {"note": "HPXML format detected but extraction not yet implemented"}
...
def validate_energy_data(energy_data: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if record_count < 5:
        return False, f"Only {record_count} energy records found, expected more for a complete simulation"
