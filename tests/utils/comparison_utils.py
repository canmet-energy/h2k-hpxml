    ...
    def compare_recursive(baseline: Any, current: Any, current_path: str) -> None:
        if isinstance(baseline, dict) and isinstance(current, dict):
            ...
            if 'value' in baseline and 'value' in current:
                ...
                passed = abs(pct_diff) <= tolerance_percent or (baseline_val == 0 and current_val == 0)
...
def generate_comparison_summary(comparisons: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    significant_failures = [c for c in tolerance_failures if abs(c.get('percentage_difference', 0)) > 10]
    ...
    summary = {
        ...
        'worst_differences': sorted(
            [c for c in comparisons if not c['passed'] and abs(c.get('percentage_difference', 0)) != float('inf')],
            key=lambda x: abs(x.get('percentage_difference', 0)),
            reverse=True
        )[:5]  # Top 5 worst differences
    }
...
def validate_comparison_data(baseline_data: Dict[str, Any], current_data: Dict[str, Any]) -> Tuple[bool, str]:
    """
    ...
    if count_diff_pct > 50:
        return False, f"Data structure mismatch: baseline has {baseline_count} values, current has {current_count} ({count_diff_pct:.1f}% difference)"
    
    return True, f"Validation passed: {baseline_count} baseline values, {current_count} current values"
