...
def test_demo_ruby_hpxml_path_construction():
    """Test that ruby_hpxml_path is constructed correctly (not from config)."""
    # This is how demo.py should construct the path (fixed version)
    ...
    assert hpxml_os_path is not None, "hpxml_os_path should be in config"
...
def test_config_does_not_have_ruby_hpxml_path():
    ...
    assert ruby_hpxml_path_from_config is None, (
        "Config should not contain ruby_hpxml_path key - it should be constructed from hpxml_os_path"
    )