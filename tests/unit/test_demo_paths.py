...
These tests are hermetic: the OpenStudio-HPXML path is mocked (via
get_hpxml_os_path) so they do not require an installed toolchain.
"""
...
def test_demo_ruby_hpxml_path_construction(tmp_path):
    """Test that ruby_hpxml_path is constructed correctly using auto-detection."""
    # Build a fake OpenStudio-HPXML install with the workflow script present.
    fake_hpxml = tmp_path / "OpenStudio-HPXML"
    (fake_hpxml / "workflow").mkdir(parents=True)
    (fake_hpxml / "workflow" / "run_simulation.rb").write_text("# stub workflow script")

    with patch("h2k_hpxml.utils.dependencies.get_hpxml_os_path", return_value=str(fake_hpxml)):
        ...
        assert "workflow/run_simulation.rb" in ruby_hpxml_path.replace(
            os.sep, "/"
        ), "Should contain workflow script path"
