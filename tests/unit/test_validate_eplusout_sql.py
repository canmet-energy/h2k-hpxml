...
def test_validate_baseline_summary():
    ...
    summary_file = "tests/fixtures/expected_outputs/golden_files/baseline/baseline_energy_summary.json"
    ...
    for file_name, result_data in summary_data["results"].items():
        ...
        assert os.path.exists(baseline_path), f"Individual baseline file '{baseline_path}' does not exist"
...
def test_validate_error_logs():
    ...
    if not os.path.exists(temp_dir):
        pytest.skip(f"Temp directory '{temp_dir}' does not exist - run tests first to generate simulation outputs")
            ...
            for subdir in os.listdir(dir_path):
                ...
                if os.path.isdir(subdir_path):
                    ...
                    if os.path.exists(err_path):
                        ...
                        assert len(fatal_errors) == 0, f"Fatal errors found in {err_path}: {fatal_errors}"
                        print(f"✓ No fatal errors in: {err_path}")
    
    if not found_error_files:
        print("ℹ️  No error files found in temp directory - simulation outputs may have been cleaned up")
