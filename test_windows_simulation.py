            ...
            with patch("click.confirm", mock_confirm):
                with patch("webbrowser.open", mock_browser):
                    ...
                    if browser_called:
                        ...
                        print(
                            f"✅ URL contains version: {manager.REQUIRED_OPENSTUDIO_VERSION in url}"
                        )
                    ...
                    with patch("pathlib.Path.exists", return_value=False):
                        with patch("pathlib.Path.mkdir"):
                            ...
                            with patch("pathlib.Path.iterdir", return_value=[mock_folder]):
                                with patch(
                                    "h2k_hpxml.utils.dependencies.DependencyManager._update_config_file",
                                    return_value=True,
                                ) as mock_config:
                                    ...
                                    print(
                                        f"✅ Windows path used: {str(manager.default_hpxml_path) == 'C:/OpenStudio-HPXML'}"
                                    )
