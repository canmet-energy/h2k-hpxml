...
# Install system dependencies required by OpenStudio (when pip installs it)
RUN apt-get update && apt-get install -y \
    # Basic utilities
    curl \
    git \
...
RUN mkdir -p /app/config/templates /app/deps \
...
# Let the Python package manage these paths
...
# Ensure /app exists and setup permissions with deps directory
