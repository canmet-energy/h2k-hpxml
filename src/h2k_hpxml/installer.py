...
def install_openstudio_hpxml(force=False):
    ...
    try:
        ...
        if nested_dir.exists() and nested_dir.is_dir():
            ...
            print(f"  ⚠ Detected nested structure, fixing...")
            ...
            potential_dirs = [d for d in DEPS_DIR.iterdir() if d.is_dir() and "hpxml" in d.name.lower()]
            if potential_dirs:
        ...
        if not workflow_file.exists():
            raise RuntimeError(f"OpenStudio-HPXML extraction failed - missing workflow file at {workflow_file}")
    ...
    if system == "Windows":
        ...
        try:
            ...
            result = subprocess.run([
                "msiexec", "/a", str(exe_path), "/quiet", 
                f"TARGETDIR={OPENSTUDIO_DIR}"
        ...
        print(f"  ⚠️  Automatic extraction failed. Manual steps required:")
        ...
        print(f"  Then run 'h2k-deps' again to verify installation.")
        return False
...
def _check_and_cleanup_orphaned_deps():
    ...
    try:
        ...
        if installed_deps_version != current_deps_version:
            print(f"Note: Dependency versions have changed, updating...")
