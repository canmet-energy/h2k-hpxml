...
def _build_simulation_flags(add_component_loads, debug, skip_validation, 
                          output_format, timestep, daily, hourly, monthly,
                          add_stochastic_schedules, add_timeseries_output_variable):
    """
    ...
    flags = " ".join(
        ...
        for key, value in flag_options.items() if value
    )

    # Add options that can be repeated
    repeated_options = [
        ("--timestep", timestep), ("--hourly", hourly), 
    ...
    if add_timeseries_output_variable:
        flags += " " + " ".join(
            f"--add-timeseries-output-variable {v}" 
            for v in add_timeseries_output_variable
        )
...
def _convert_h2k_to_hpxml(filepath, dest_hpxml_path):
    ...
    hpxml_path = os.path.join(
        dest_hpxml_path, file_stem, f"{file_stem}.xml"
    )
...
@click.option(
    '--output_path', '-o',
    help=('Path to output hpxml files. By default it is the same as the '
          'input path with a folder named output created inside it.')
)
@click.option(
    '--timestep', multiple=True, default=[],
    help=('Request timestep output type (ALL, total, fuels, enduses, '
...
@click.option(
    '--daily', multiple=True, default=[],
    help=('Request daily output type (ALL, total, fuels, enduses, '
          'systemuses, emissions, emissionfuels, emissionenduses, hotwater, '
          'loads, componentloads, unmethours, temperatures, airflows, '
          'weather, resilience); can be called multiple times')
)
@click.option(
    '--hourly', multiple=True, default=[],
    help=('Request hourly output type (ALL, total, fuels, enduses, '
          'systemuses, emissions, emissionfuels, emissionenduses, hotwater, '
          'loads, componentloads, unmethours, temperatures, airflows, '
          'weather, resilience); can be called multiple times')
)
@click.option(
    '--monthly', multiple=True, default=[],
    help=('Request monthly output type (ALL, total, fuels, enduses, '
          'systemuses, emissions, emissionfuels, emissionenduses, hotwater, '
          'loads, componentloads, unmethours, temperatures, airflows, '
          'weather, resilience); can be called multiple times')
)
@click.option(
    '--add-component-loads', '-l', is_flag=True, default=True,
...
@click.option(
    '--skip-validation', '-s', is_flag=True, default=False,
    help='Skip Schema/Schematron validation for faster performance'
)
@click.option(
    '--output-format', '-f', default='csv',
    help=('Output format for the simulation results '
          '(csv, json, msgpack, csv_dview)')
)
@click.option(
    '--add-stochastic-schedules', is_flag=True, default=False,
    help='Add detailed stochastic occupancy schedules'
)
@click.option(
    '--add-timeseries-output-variable', multiple=True, default=[],
    help='Add timeseries output variable; can be called multiple times'
)
@click.option(
    '--do-not-sim', is_flag=True, default=False,
    help='Convert only, do not run simulation'
)
def run(input_path, output_path, timestep, daily, hourly, monthly,
        add_component_loads, debug, skip_validation, output_format,
        add_stochastic_schedules, add_timeseries_output_variable, do_not_sim):
    """
    ...
    flags = _build_simulation_flags(
        add_component_loads, debug, skip_validation, output_format,
        timestep, daily, hourly, monthly, add_stochastic_schedules,
    ...
    if os.path.isfile(source_h2k_path) and source_h2k_path.lower().endswith(".h2k"):
        ...
        print(
            f"The source path {source_h2k_path} is neither a .h2k file "
            f"nor a directory."
        )
        sys.exit(1)
            ...
            if not do_not_sim:
                ...
                if status == "Success":
                    ...
                    error_details = _handle_processing_error(
                        filepath, dest_hpxml_path, 
                        subprocess.CalledProcessError(1, "simulation", error_msg), tb
                    )
