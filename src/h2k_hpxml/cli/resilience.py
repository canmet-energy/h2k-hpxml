...
@click.option(
    '--outage-days', default=DEFAULT_OUTAGE_DAYS, type=click.IntRange(0, 365),
    help=('Number of days for power outage analysis '
...
@click.option(
    '--clothing-factor-summer', default=DEFAULT_SUMMER_CLOTHING,
    type=click.FloatRange(0.0, 2.0),
    help=('Summer clothing insulation factor '
          '(default: 0.5, range: 0.0-2.0)')
)
@click.option(
    '--clothing-factor-winter', default=DEFAULT_WINTER_CLOTHING,
    type=click.FloatRange(0.0, 2.0),
    help=('Winter clothing insulation factor '
          '(default: 1.0, range: 0.0-2.0)')
)
@click.option(
    '--run-simulation', is_flag=True, default=False,
    help='Run the OpenStudio simulations for all scenarios (default: False)'
)
@click.version_option(version='1.0.0')
def resilience(h2k_path, outage_days, output_path, clothing_factor_summer,
               clothing_factor_winter, run_simulation):
    """
    ...
    try:
        ...
        if not OPENSTUDIO_AVAILABLE:
            ...
            click.echo(
                "Please ensure OpenStudio is properly installed with Python bindings.",
                err=True
            )
            sys.exit(1)
...
class ResilienceProcessor:
    ...
    def __init__(self, h2k_path, output_path, outage_days, clothing_factor_summer,
                 clothing_factor_winter, run_simulation=False):
    ...
    def analyze_summer_period(self, weather_file_path, weather_type):
        ...
        try:
            ...
            if summer_periods:
                ...
                return {
                    'start': longest_period['start'],
                    'end': longest_period['end']
                }
            else:
                # Fallback to meteorological summer
                click.echo(f"Warning: No clear summer period found in {weather_type}, using meteorological summer")
                return {
    ...
    def _create_clothing_schedule(self, model):
        ...
        try:
            ...
            if hasattr(self, 'summer_periods'):
                ...
                summer_rule.setStartDate(openstudio.Date(
                    openstudio.MonthOfYear(summer_start.month),
                    summer_start.day
                ))
                summer_rule.setEndDate(openstudio.Date(
                    openstudio.MonthOfYear(summer_end.month),
                    summer_end.day
                ))
            else:
                # Fallback to default summer period
                summer_rule.setStartDate(openstudio.Date(
                    openstudio.MonthOfYear(SUMMER_START_DEFAULT[0]),
                    SUMMER_START_DEFAULT[1]
                ))
                summer_rule.setEndDate(openstudio.Date(
                    openstudio.MonthOfYear(SUMMER_END_DEFAULT[0]),
                    SUMMER_END_DEFAULT[1]
                ))
            ...
            for people_def in model.getPeopleDefinitions():
                if hasattr(people_def, 'setClothingInsulationSchedule'):
                    ...
                    click.echo("    Warning: Clothing insulation schedule API not available in this OpenStudio version")
            
            click.echo(f"    Created seasonal clothing schedule (Summer: {self.clothing_factor_summer}, Winter: {self.clothing_factor_winter})")
    ...
    def _create_power_failure_schedule(self, model, weather_type):
        ...
        try:
            ...
            outage_rule.setStartDate(openstudio.Date(
                openstudio.MonthOfYear(outage_start.month),
                outage_start.day
            ))
            outage_rule.setEndDate(openstudio.Date(
                openstudio.MonthOfYear(outage_end.month),
                outage_end.day
            ))
            ...
            click.echo(f"    Created power failure schedule: {outage_start.strftime('%Y-%m-%d')} to {outage_end.strftime('%Y-%m-%d')}")
        ...
        try:
            ...
            for scenario_name, config in scenarios.items():
                ...
                scenario_path = os.path.join(
                    self.scenario_paths[scenario_name], 
                    f"{scenario_name}.osm"
                )
                    ...
                    with open(err_path, 'r') as err_file:
                        ...
                        if completion_line:
                            ...
                            if severe_match:
                                ...
                                if severe_count > 0:
                                    log_file.write(f"ERROR: {severe_count} severe errors in simulation\n")
                            ...
                            if actual_fatal or actual_severe:
                                log_file.write("ERROR: Fatal or severe errors found in simulation:\n")
                                error_lines = []
                                for line in err_content.split('\n'):
