...
def get_numbers_and_provinces(
    file_path="/workspaces/h2k_hpxml/src/h2k_hpxml/resources/weather/h2k_weather_names.txt",
):
    ...
    with open(
        os.path.join(os.path.dirname(__file__), "..", "resources", "weather", "historic.json"),
    ) as f:
    ...
    province_df = pd.DataFrame(
        {"provinces_english": provinces_english, "provinces_french": provinces_french}
    )
    ...
    merged_df = city_df.merge(
        province_df, left_on="provinces_city_map", right_index=True, how="left"
    )
    print(merged_df)
    ...
    for province in prov_terr_codes:
        ...
        filtered_cwec_files_by_province = [
            file for file in canadian_cwec_files if f"CAN_{prov_terr_codes[province]}" in file
        ]
        ...
        for historic_type in historic_types:
            ...
            filtered_cwec_files = [
                file for file in filtered_cwec_files_by_province if historic_type in file
            ]
            print(historic_type)
