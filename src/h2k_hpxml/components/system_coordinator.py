...
from .system_additional_openings import get_additional_openings
from .system_air_conditioning import get_air_conditioning
from .system_heat_pumps import get_heat_pump
from .system_heating_primary import get_primary_heating_system
from .system_heating_secondary import get_secondary_heating_systems
from .system_hot_water import get_hot_water_systems
from .system_hot_water_distribution import get_hot_water_distribution
from .system_hvac_control import get_hvac_control
from .system_hvac_distribution import get_hvac_distribution
from .system_solar_generation import get_solar_generation
from .system_ventilation import get_ventilation_systems
from .system_water_fixtures import get_water_fixtures
...
def get_systems(h2k_dict, model_data):
    ...
    model_data.set_system_id({"hvac_hydronic_distribution": "HVACHydronicDistribution1"})
    ...
    secondary_heating_systems, remaining_fraction_floor_area = get_secondary_heating_systems(
        h2k_dict, model_data
    )
    ...
    get_additional_openings(h2k_dict, model_data)
    ...
    if heat_pump_backup_type == "separate":
        ...
        heat_pump_result["FractionHeatLoadServed"] = max(0, remaining_fraction_floor_area)
    elif heat_pump_backup_type == "integrated":
        heat_pump_result["FractionHeatLoadServed"] = max(0, remaining_fraction_floor_area)
    else:
        primary_heating_result["FractionHeatLoadServed"] = max(0, remaining_fraction_floor_area)
        ...
        "HVACPlant": {
            "PrimarySystems": {
                ...
                **(
                    ...
                    if primary_cooling_id is not None
                    else {}
                ),
            },
            **(
            ...
            **({"CoolingSystem": air_conditioning_result} if air_conditioning_result != {} else {}),
        },
        "HVACControl": hvac_control_result,
        **(
            {"HVACDistribution": hvac_distribution_result} if hvac_distribution_result != {} else {}
        ),
    }

    hot_water_system_result, solar_dhw_system_result = get_hot_water_systems(h2k_dict, model_data)
    ...
    solar_dhw_dict = (
        {"SolarThermalSystem": solar_dhw_system_result} if solar_dhw_system_result != {} else {}
    )
