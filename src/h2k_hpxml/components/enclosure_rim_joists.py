...
def get_rim_joists(h2k_floor_headers, parent_type, model_data):
    ...
    h2k_floor_headers = (
        h2k_floor_headers if isinstance(h2k_floor_headers, list) else [h2k_floor_headers]
    )
    ...
    for floor_header in h2k_floor_headers:
        ...
        rim_joist_perimeter = h2k.get_number_field(floor_header, "floor_header_perimeter")
        ...
        new_rim_joists = {
            ...
            "InteriorAdjacentTo": (
                "basement - conditioned" if parent_type == "Basement" else "conditioned space"
            ),
            "Area": rim_joist_area,
            "Insulation": {
