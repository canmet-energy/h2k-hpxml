...
from .enclosure_skylights import get_skylights


# Gets all wall components and possible subcomponents (windows, doors, floor headers)
def get_ceilings(h2k_dict, model_data=None):
    ...
    for ceiling in h2k_ceilings:
        ...
        if construction_type.lower() == "attic/gable":
            ...
            if roof_pitch is None:
                roof_pitch = int(round(12 * h2k.get_number_field(ceiling, "roof_pitch_value")))
            ...
            if has_roof_cavity_inputs:
                ...
                gable_wall_rval = gable_ends_sheathing_r_value + gable_ends_exterior_r_value
            ...
            new_floor = {
                ...
                "InteriorFinish": {"Type": "gypsum board"},  # default for ceiling floors
                "Insulation": {
            ...
            if ceiling_length < perimeter_min:
                ...
                width = (ceiling_length + math.sqrt(ceiling_length**2 - (8 * ceiling_area))) / 4
                length = ceiling_area / width
