...
def get_solar_generation(h2k_dict, model_data):
    ...
    h2k_solar_pv_systems = (
        h2k_solar_pv_systems if isinstance(h2k_solar_pv_systems, list) else [h2k_solar_pv_systems]
    )
...
def get_solar_pv_system(sys_data, ind, model_data):
    ...
    system_losses_fraction = 1 - (1 * (1 - pv_array_misc_losses) * (1 - pv_array_other_losses))
