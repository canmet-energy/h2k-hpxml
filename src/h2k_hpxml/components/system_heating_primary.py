...
def get_electric_resistance(type1_data, model_data):
    ...
    baseboard_sizing_factor = h2k.get_number_field(type1_data, "baseboard_sizing_factor")
    is_auto_sized = (
    ...
    elec_resistance = {
        ...
        "HeatingSystemType": {"ElectricResistance": {"ElectricDistribution": "baseboard"}},
...
def get_boiler(type1_data, model_data):
    ...
    if "ComboTankAndPump" in type1_data.keys():
        ...
        if combo_default_ef:
            ...
            combo_energy_factor = float(
                type1_data.get("ComboTankAndPump", {}).get("EnergyFactor", {}).get("@value", "0")
            )
    ...
    if results != {}:
        ...
        if boiler_fuel_type != "electricity" and heat_pump_elec_heating_GJ > 0:
            ...
            electric_aux_energy = max(0, tot_elec_heating_GJ - heat_pump_elec_heating_GJ) * (
                1 / 0.0036
            )
...
def get_stove(type1_data, model_data):
    ...
    stove_dict = {
        ...
        **({} if is_auto_sized else {"HeatingCapacity": stove_capacity}),
        ...
        "FractionHeatLoadServed": 1,
        **(
            {"extension": {"HeatingAutosizingFactor": stove_sizing_factor}} if is_auto_sized else {}
        ),
    }
...
def get_p9_heating_system(type1_data, model_data):
    ...
    if results != {}:
        ...
        electric_aux_energy = max(0, tot_elec_heating_GJ - primary_elec_heating_GJ) * (1 / 0.0036)
