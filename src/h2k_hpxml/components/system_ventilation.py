...
def get_ventilation_systems(h2k_dict, model_data):
    ...
    whole_house_ventilators = {} if whole_house_ventilators is None else whole_house_ventilators
    ...
    supplemental_ventilators = {} if supplemental_ventilators is None else supplemental_ventilators
    ...
    for whole_vent in whole_house_base_vent:
        whole_vent_res = get_ventilator(whole_vent, i, vent_hours_per_day, True, model_data)
        ventilation_fan_list = [*ventilation_fan_list, whole_vent_res]
        i += 1

    for suppl_vent in supplemental_base_vent:
        suppl_vent_res = get_ventilator(suppl_vent, i, vent_hours_per_day, False, model_data)
...
def get_ventilator(sys_data, ind, vent_hours_per_day, is_whole_house, model_data):
    ...
    supplemental_vent_hours_per_day = h2k.get_number_field(sys_data, "supplemental_vent_schedule")
    ...
    if abs(supply_flow_rate - exhaust_flow_rate) / max(supply_flow_rate, exhaust_flow_rate) <= 0.05:
...
def get_hrv(sys_data, vent_hours_per_day, is_whole_house, model_data):
    ...
    hrv = {
        "SystemIdentifier": {"@id": f"Hrv{'WholeHouse' if is_whole_house else 'Supplemental'}1"},
