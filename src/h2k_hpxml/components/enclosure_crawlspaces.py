...
def get_crawlspaces(h2k_dict, model_data=None):
    ...
    h2k.get_number_field(h2k_dict, "wall_absorptance")
    is_crawlspace_heated = h2k.get_selection_field(h2k_dict, "crawlspace_heated")

    # Always process as array
    h2k_crawlspaces = h2k_crawlspaces if isinstance(h2k_crawlspaces, list) else [h2k_crawlspaces]
    ...
    for crawlspace in h2k_crawlspaces:
        ...
        interior_adjacent = "outside" if is_open_crawlspace else "crawlspace - conditioned"
        ...
        floors_above_rval = h2k.get_number_field(crawlspace, "floors_above_foundation_rval")

        slab_r_val = 0  # nominal R-value, so not including concrete
        insulation_spans_slab = True
        slab_ins_width = round(math.sqrt(foundation_tot_area) / 2, 2)  # default to "full"
        ...
        new_foundation = {
            ...
            **({"AttachedToSlab": {"@idref": slab_id}} if not is_open_crawlspace else {}),
